...
public class VerifyConfig {
	...
	@Autowired
	public void setConfig(Config config, UserService service){
		VerifyConfig.config = config;
		VerifyConfig.service = service;
	}
	...
	public static User getUserInfo(HttpServletRequest req){
		...
		//Test
		String test = Tools.getReqParamSafe(req, "test");
		if(!Tools.isNullOrEmpty(test)){
			user = new User();
			user.setStaffId("CD0436");
		}
	...
	public static synchronized void setUserInfo(HttpServletRequest req, Map<String, Object> ssoUser){
		...
		String staffId = ssoUser.get("staffId")+"";
		User test = service.findOne(MapTools.custom().put("staffId", staffId).build());
		if(test != null){
			//转为本地用户，加入缓存
			HttpSession session = req.getSession();
			session.setAttribute(config.login_session_key, test);
		}
